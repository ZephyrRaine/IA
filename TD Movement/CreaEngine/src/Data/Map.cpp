...
namespace crea
{
	...
	bool Map::loadFromFileJSON(string& _filename)
	{
		...
		for (unsigned int iTileset = 0; iTileset < tilesets.size(); ++iTileset)
		{
			...
			pTileSet->m_pSprite->setOrigin(pTileSet->m_nTilewidth*0.5f, pTileSet->m_nTileheight*0.5f);
		...
		for (unsigned int iLayer = 0; iLayer < layers.size(); ++iLayer)
		{
			...
			if (type == "objectgroup")
			{
				...
				for (unsigned int iObject = 0; iObject < objects.size(); ++iObject)
				{
					...
					pEntity->setPosition(Vector2f((float)iPersoX + iPersoWidth*0.5f, (float)(iPersoY - iPersoHeight*0.5f)));
	...
	bool Map::draw()
	{
		...
		for (short i = m_iMin; i <= m_iMax; i++)
		{
			...
			for (short j = m_jMin; j <= m_jMax; j++)
			{
				...
				pTileSet->m_pSprite->setPosition((float)pTileSet->m_nTilewidth*(i+0.5f), (float)pTileSet->m_nTileheight*(j+0.5f));
